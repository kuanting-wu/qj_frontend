<template>
  <main
    ...
    <div v-if="notification.show" class="mt-4 w-80 p-4 rounded-lg text-center transition-all duration-200" :class="[
    ...
    <form @submit.prevent="handleSignIn"
      ...
      <div class="flex flex-col w-full whitespace-nowrap">
        <label for="email" class="leading-snug text-gray-700 dark:text-gray-300 transition-colors duration-200">
          Email
        </label>
      ...
      <div class="flex flex-col mt-6 w-full whitespace-nowrap">
        <label for="password" class="leading-snug text-gray-700 dark:text-gray-300 transition-colors duration-200">
          Password
        </label>
      ...
      <div class="flex gap-4 items-center mt-6 w-full leading-none">
    ...
    const handleSignIn = async () => {
      ...
      try {
        ...
        if (response.status === 200) {
          const { accessToken, refreshToken, message, email_verified } = response.data;
          ...
          // Use route.query.redirect if present, otherwise use previousRoute, fallback to "/"
          const redirectTo = route.query.redirect || previousRoute || "/";
